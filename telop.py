...
def set_telop(text):
    ...
    def telop(img, message, W, H):
        font_path = '/Library/Fonts/Arial Unicode.ttf'        # Windowsのフォントファイルへのパス
        ...
        draw.text(position, message, font=font, fill=(0, 0, 0, 0))
    ...
    dir = 'video'               # 動画が保存されているディレクトリ
    path = 'test_video.mp4'   # ファイル名
    ...
    sentences = text.split('。')            # テロップ文章を分割
    for i in range(len(sentences)):
        # テロップ文章と時間の配列を作成
        time = spc * len(sentences[i])
        message.append([sentences[i], sum_time, sum_time + time])
